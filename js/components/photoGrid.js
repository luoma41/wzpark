  ...
  render(photos, options = {}) {
    ...
    this.container.innerHTML = `
      <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
        ${photos.map((p, idx) => this.photoCard(p, editable, idx)).join('')}
      </div>
    `;
  ...
  photoCard(photo, editable, idx) {
    return `
      <div class="group relative aspect-square overflow-hidden rounded-lg bg-sand/20">
        ...
        ` : ''}
        ${editable ? `
          <button data-id="${photo._id}" class="delete-btn absolute top-2 right-2 w-6 h-6 bg-red-500 text-white rounded-full text-xs opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
            ×
          </button>
        ` : ''}
      </div>
